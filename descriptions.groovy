Hubitat app code best experience. Hallway Lights

DEVICES:
- main lights 
- side lights
- virtual device, to indicate it is dark 
- lux sensor
- motion sensors to turn on lights
- motion sensors to keep on lights
- room door
- house door
...
Per mode different configurable options
- Bool Do in mode
- bool lights on in mode
- brightness
- color (if color bulb)
- stayon time

logging in debug and info, with postfix the method name and prefix the [roomname]
Allow easy testing, option to change mode in app

Specifics
All chosen modes and when virtual dark device is on, or lux is low
When motion, turn main lights for configured stayon and brightness. If no motion after period then turn off. If still motion after stayon time, wait another stayon time en so forth

When mode = sleepall and motion
If room door = open turn main lights off, and side lights on. If door closes turn main lights on and  side lights off. 

Mode Normal
House door, if opened full brightness for 5 minutes and if no motion then turn off. If still motion after 5 minutes, wait another minute en so forth

Mode Sleepsome
House door, if opened on a weekday between 4 and 5 am, turn off all lights

Button
Button press low brightness, double click full brightness, on for 5 minutes and if no motion then turn off. If still motion after 5 minutes, wait another minute en so forth

Finally
Make all devices input (select multiple devices) configureable.
